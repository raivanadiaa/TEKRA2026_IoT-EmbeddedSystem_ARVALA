...
const char* MQTT_TOPIC    = "smartclassroom/kelas2";
...
void connectMQTT() {
  while (!mqtt.connected()) {
    ...
    String clientId = "ESP32-Kelas2-" + String(random(0xffff), HEX);
...
void kirimMQTT(bool pir, float suhu, float kelembapan, int ldr, int mq) {
  ...
  if (mqtt.publish(MQTT_TOPIC, payload)) Serial.println("[MQTT] Terkirim - Kelas 2");
}
